...
const mockedResolveRouteParams = vi.mocked(resolveRouteParams);
const mockedUpstreamUnavailable = vi.mocked(upstreamUnavailable);

describe('api/admin/nomina/payroll/jobs/run/[fileId] POST', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    ...
    mockedResolveRouteParams.mockResolvedValue({ fileId: '21' });
  ...
  it('rejects invalid route params before calling the upstream', async () => {
    ...
    mockedResolveRouteParams.mockResolvedValueOnce({ fileId: 'bad' });
  ...
  it('forwards a valid payroll execution request to batch', async () => {
    mockedRequireAdminAccess.mockResolvedValueOnce(null);
    ...
    const response = await POST(request, {
      params: Promise.resolve({ fileId: '21' }),
    });

    expect(mockedResolveRouteParams).toHaveBeenCalled();
    expect(mockedBuildProxyHeaders).toHaveBeenCalledWith({ req: request });
    expect(global.fetch).toHaveBeenCalledWith(
      'http://batch.local/api/admin/nomina/payroll/jobs/run/21',
